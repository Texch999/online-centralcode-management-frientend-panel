...
import { createDirector, getAdminWebsites, getCountries, getDirectorDetailsById, getUserWebsites } from "../../../api/apiMethods";
import { adminRoles, Roles } from "../../../utils/enum";

function EditDirectorAdminPopup({ showEditModal, handleEditModalClose, directorId }) {
    ...
    useEffect(() => { GetIndividualDirectors() }, [directorId])
    ...
    useEffect(() => {
        if (directorData) {
            setFormData({
                ...
                managementPassword: "",
                accessWebsites: directorData.accessWebsites.map((website) => ({
                    ...website,
                    accountType: website.share !== null ? "share" : "rental",
                })) || [],
            });

            setSelectedRole(adminRoless.find(role => role.value === directorData.type) || null);
            setSelectedCountry(countryData?.find(country => country.id === directorData.county)?.code || "");
        }
    ...
    return (
        <Modal show={showEditModal} onHide={handleEditModalClose} centered size="md">
            <Modal.Body className="p-1 director-admin-popupbody px-2 py-2">
                ...
                {formData.accessWebsites.map((website, index) => (
                    <form key={index} className="custom-form small-font p-3" style={{ display: activeForm === 2 ? "block" : "none" }}>
                                ...
                                <div className="row mb-3">
                                    ...
                                    <div className="col-md-6">
                                        ...
                                        <input
                                            ...
                                            onChange={(e) => {
                                                const updatedWebsites = [...formData.accessWebsites];
                                                updatedWebsites[index].rent_expiry_date = e.target.value;
                                                setFormData({ ...formData, accessWebsites: updatedWebsites });
                                            }}
                                        />
                                    </div>
