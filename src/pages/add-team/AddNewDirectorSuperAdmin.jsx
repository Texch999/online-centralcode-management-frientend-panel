...
function AddNewDirectorSuperAdmin() {
  ...
  const [validationErrors, setValidationErrors] = useState({});
  const [selectedUserSitesByAdmin, setSelectedUserSitesByAdmin] = useState({});
  const [lastSelectedAdmin, setLastSelectedAdmin] = useState(null);
  const [firstSelectedAdmin, setFirstSelectedAdmin] = useState(null); // Track first selected admin for SuperAdmin
  const [loader, setLoader] = useState(false);
  ...
  const handleAdminRoleChange = (formId, selectedOption) => {
    ...
    // If role is SuperAdmin (2), set the first selected admin and prevent changing it
    if (selectedRole === "2" && !firstSelectedAdmin) {
      setFirstSelectedAdmin(selectedOption);
    }
  ...
  const handleSubmit = (e) => {
    ...
    const selectedUserWebsites = forms.map((form) => {
      ...
      // For SuperAdmin, use the first selected admin for all forms
      const adminId =
        selectedRole === "2"
          ? firstSelectedAdmin?.value
          : selectedAdmins[formId]?.value;

      let websiteData = {
        admin_panel_id: adminId,
        user_paner_id: userSite.id,
        commission_type: accotypeid,
      };
    ...
    if (depositRemark.value == "credit" && isCreditAllowed) {
      finalData.creditAmount = enteredPaidAmount
        ? parseFloat(addDepositChips) - parseFloat(enteredPaidAmount)
        : parseFloat(addDepositChips);
      finalData.offDepositAmount = enteredPaidAmount
        ? parseFloat(enteredPaidAmount)
        : 0;
      finalData.depositType = 1;
    } else {
      finalData.depositType = 2;
  ...
  const addAnotherForm = () => {
    ...
    // If role is SuperAdmin (2) and we have a first selected admin, auto-select it
    if (selectedRole === "2" && firstSelectedAdmin) {
      setSelectedAdmins((prev) => ({
        ...prev,
        [newFormId]: firstSelectedAdmin,
      }));

      const adminData = adminWebsite.find(
        (admin) => admin.id === firstSelectedAdmin.value
      );
      setUserWebsitesList((prev) => ({
        ...prev,
        [newFormId]: adminData?.userWebsites || [],
    ...
    // For other roles or if no first selected admin yet, use last selected admin
    else if (lastSelectedAdmin) {
      setSelectedAdmins((prev) => ({
        ...prev,
        [newFormId]: lastSelectedAdmin,
      }));
          ...
          <div className="row">
            ...
            <div className="col p-1">
              ...
              <Select
                ...
                onChange={(selectedOption) => {
                  setSelectedRole(selectedOption?.value);
                  // Reset first selected admin when role changes
                  if (selectedOption?.value !== "2") {
                    setFirstSelectedAdmin(null);
                  }
                }}
                styles={customStyles}
              />
                ...
                <div className="col-2 ">
                  ...
                  <div className="custom-select-wrapper">
                    <Select
                      ...
                      value={
                        // For SuperAdmin role, use the first selected admin if it exists
                        selectedRole === "2" && firstSelectedAdmin
                          ? firstSelectedAdmin
                          : selectedAdmins[form.id] || null
                      }
                      onChange={(selectedOption) =>
                        handleAdminRoleChange(form.id, selectedOption)
                      }
                      isDisabled={
                        // Disable if role is SuperAdmin and we have a first selected admin
                        selectedRole === "2" && firstSelectedAdmin !== null
                      }
                    />
                  </div>
                </div>
                                  ...
                                  <div className="rounded input-css d-flex justify-content-between align-items-center small-font">
                                    <input
                                      ...
                                      onChange={(e) => {
                                        ...
                                        // Allow numbers between 0-100 with up to 2 decimal places
                                        if (
                                          /^([0-9]{1,2}(\.\d{0,2})?|100(\.0{0,2})?)?$/.test(
                                            value
                                          )
                                        ) {
                                          let numericValue = parseFloat(value);
