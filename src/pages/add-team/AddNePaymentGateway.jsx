...
const AddNePaymentGateway = () => {
    ...
    const filteredPaymentModes = paymentModes.filter((mode) => mode.country_id === selectedCountryId);
                ...
                <div className="row mb-3">
                    <div className="col-3">
                        ...
                        <Select
                            ...
                            maxMenuHeight={300}
                            menuPlacement="auto"
                            value={formattedCountries.find(option => option.value === selectedCountryId)}
                            onChange={(selected) => setSelectedCountryId(selected.value)}
                            formatOptionLabel={(option) => (
                                <div style={{ display: "flex", justifyContent: "space-between", textTransform: "text-capitalize" }}>
                ...
                {hasNoRecords ? (
                    ...
                    <>
                        ...
                        {modes
                            ...
                            .map(({ title, mode }) => {
                                const filteredPayments = filteredPaymentModes?.filter((card) => card.avil_modes === mode);
                                return (
                                                ...
                                                {filteredPayments.map((card) => (
                                                    <div key={card.id} className="col-2">
                                                        <div className="card h-100">
                                                            ...
                                                            >
                                                                <span className="text-center large-font">{card.name}</span>
                                                            </div>
                                                        </div>
                                                    </div>
                                                ))}
