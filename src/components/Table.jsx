...
//     // Call onPageChange with limit and offset
//     if (onPageChange) {
...
function Table({
  ...
  return (
    <div>
      {/* Table */}
      <div className="w-100 table-wrapper scroll-x">
        <table className="w-100 fixed-table white-bg" style={{ borderRadius: "10px" }}>
          <thead>
            <tr className="border-bottom">
              {columns.map((column, index) => (
                <th key={index} style={{ width: column.width }} className={`border-bottom small-font fw-600 black-text ${customPadding}`}>
                  {column.header}
                </th>
              ))}
            </tr>
          ...
          <tbody>
            {hasData ? (
              data.map((row, rowIndex) => (
                <tr key={rowIndex} className="border-top">
                  {columns.map((column, colIndex) => (
                    <td key={colIndex} style={{ width: column.width }} className={`${rowColor ? rowColor(row) : "black-text"} align-top small-font ${customPadding}`}>
                      {row[column.field]}
                    </td>
                  ))}
                </tr>
              ...
              <tr className="border-top">
                <td colSpan={columns.length} className="text-center black-text p-2">
                  <h6 className="mb-0">No Data Available</h6>
                </td>
              </tr>
            )}
          ...
          {footer.length > 0 && (
            <tfoot className="border-top footer-bg">
              <tr>
                {footer.map((column, index) => (
                  <th key={index} style={{ width: column.width }} className="small-font fw-600 black-text px-3 py-3">
                    {column.header}
                  </th>
                ))}
              </tr>
